...
%que abordamos en este Trabajo de Fin de Máster.
En particular, nos centraremos en cómo la literatura ha estudiado las debilidades en la generación de claves RSA, la detección de factores compartidos entre claves públicas y el análisis de este tipo de vulnerabilidades sobre grandes conjuntos de datos reales. Además, revisaremos los enfoques \cambio{computacionales} clásicos utilizados hasta ahora, sus principales limitaciones y la propuesta más reciente en la que se apoya este trabajo, con el fin de situar nuestra aportación a la literatura previa.


\section{Detección de factores compartidos en claves públicas RSA}

Como ya hemos explicado en el capítulo anterior, la seguridad práctica de RSA depende en gran medida de que la generación de claves se realice correctamente. Por ello, una de las líneas más importantes dentro de la literatura ha consistido en analizar hasta qué punto problemas de baja entropía o fallos de implementación pueden dar lugar a claves vulnerables en sistemas reales. Uno de los trabajos más influyentes en esta dirección mostró que la existencia de claves débiles no era una posibilidad puramente teórica, sino un problema que podía observarse en dispositivos conectados a Internet \citep{heninger2012mining}. A partir de este tipo de estudios quedó claro que, aunque RSA siga siendo un esquema robusto desde el punto de vista matemático, su seguridad puede verse comprometida  \cambio{por la debilidad si la fuente de entropía produce valores que no tienen una distribución estadística uniforme, sino que la frecuencia de algunos valores es mayor. Dado que  estos valores se utilizan posteriormente generación de claves, este trabajo es capaz de detectar grupos de claves generadas por los mismos fabricante solo a través del análisis posterior de éstas mediante búsqueda de factores comunes o directamente por el reuso de claves repetidas} \commA{Comprueba esto por favor y ajustalo según lo que dice Heninger}
%no dispone de suficiente aleatoriedad o si se emplean implementaciones defectuosas.
\\
\\
 \cambio{Posteriormente, }\citet{pelofske2024efficient} \cambio {profundizan en el estudio de} la fase de creación de los primos. Pelofske señala que estos fallos pueden deberse a un uso incorrecto de librerías de generación aleatoria o a la ausencia de una fuente externa de entropía suficiente en el momento de generar la clave . \citet{vage2022finding} recuerda que, en condiciones ideales, la probabilidad de que dos claves RSA distintas reutilicen un mismo primo dentro del enorme espacio de posibles primos criptográficos es prácticamente nula.  Desde el punto de vista del estado del arte, esto resulta especialmente importante porque justifica toda una línea de trabajos orientados a auditar claves públicas reales en busca de este tipo de debilidades. En otras palabras, en estudiar si las implementaciones reales respetan las condiciones necesarias para que esa seguridad teórica se mantenga en la práctica. \commA{Esto lo he resumido porque lo comentas en la introducción y sino repite mucho}
 \\

 %el interés de la literatura no ha estado en cuestionar la validez teórica de RSA, sino
 %Por ello, cuando este fenómeno aparece en la práctica, constituye una evidencia clara de que el proceso de generación ha fallado .
 
 
 %Cuando esto sucede, distintas claves pueden terminar compartiendo factores primos, lo que las vuelve inmediatamente vulnerables.


En este sentido, tanto la tesis como el artículo \cambio{del que partimos siguen la idea de que} la baja entropía y los errores de implementación pueden dar lugar a factores compartidos entre módulos RSA, que constituyen una vulnerabilidad grave que merece ser analizada empíricamente a gran escala \citep{vage2022finding,pelofske2024efficient}. A partir de aquí, el siguiente paso natural dentro de la literatura fue centrarse en cómo detectar de forma eficiente esas claves vulnerables. En este contexto, la presencia de factores compartidos entre módulos públicos se convirtió rápidamente en un objetivo de estudio especialmente relevante, ya que permite identificar claves vulnerables de manera directa.  Por ello, la cuestión dejó de ser únicamente si este fenómeno podía ocurrir y pasó a centrarse en cómo encontrar esos casos dentro de grandes colecciones de claves públicas reales
%\\
%\\
%Una vez asumido que pueden existir fallos reales en la generación de claves RSA, el siguiente problema que pasó a ocupar a la literatura fue cómo detectar esas debilidades de forma efectiva. 
%\\
%\\
%Los trabajos previos mostraron que, cuando dos módulos RSA comparten un factor primo, su factorización deja de ser un problema difícil en la práctica.. Esta transición, desde la vulnerabilidad conceptual hasta su detección empírica, marcó una parte importante del desarrollo posterior de esta línea de investigación \citep{heninger2012mining,vage2022finding}.
%\\
%\\
Tal y como plantea \citet{pelofske2024efficient}, el verdadero problema no está en calcular el máximo común divisor entre dos módulos concretos, ya que esa operación es eficiente, sino en que no se conoce de antemano qué pares de claves podrían compartir factores . Esto hace que una búsqueda exhaustiva basada en comparar todos los pares posibles se vuelva rápidamente inviable cuando el número de módulos crece. En conjuntos grandes de claves públicas, el número de comparaciones necesarias aumenta de forma cuadrática \cambio{al compararse todos los posibles pares} \commA{Yo entiendo esto, cambialo sino}, lo que convierte el enfoque \cambio{a fuerza bruta} en una opción poco realista para auditorías a gran escala \cambio{en cuanto el número de certificados a comprobar crece}.
\\
\\
...
\\
\\
Así, dentro del estado del arte, la detección de factores compartidos en claves públicas RSA debe entenderse no solo como una consecuencia natural de las debilidades en la generación de claves, sino también como un problema computacional en sí mismo \cambio{por el tamaño del espacio de búsqueda. Este trabajo tiene por tanto una doble dimensión criptográfica por un lado y algorítmica por otro que marca las contribuciones en este tema en la literatura reciente. }
%Es precisamente esa doble dimensión, criptográfica por un lado y algorítmica por otro, la que explica el interés que ha despertado este tema en la literatura reciente.
\\
\\
A partir de ahí, el siguiente paso fue \cambio{estudiar como analizar} conjuntos de datos cada vez mayores \cambio{eficientemente desde el punto de vista computacional}, con el objetivo de comprobar hasta qué punto estas debilidades aparecían realmente en sistemas desplegados en Internet.

\section{Análisis masivo de claves débiles en Internet}

\cambio{Una vez conocida la vulnerabilidad}, la investigación pasa a orientarse a cómo realizar auditorías criptográficas a gran escala, con el objetivo de comprobar la existencia de estas debilidades y \cambio{poder poner salvaguardas como la generación de nuevas claves o la revocación de certificados}.\commA{He cambiado esto porque sino es muy repetitivo con el párrafo anterior}
...
Para que este tipo de análisis masivo sea viable, la literatura tuvo que ir \cambio{más allá del enfoque de comparación} entre todos los pares posibles de módulos RSA\commA{He quitado lo de ingenuo porque naive no me traduce bien al español así pero si has visto referencia, puedes volverlo a dejar}. Como ya se ha comentado, una estrategia basada en calcular el GCD de cada par crece de forma cuadrática y se vuelve poco realista cuando el número de claves es muy grande. Por ello, se desarrollaron algoritmos específicos capaces de explotar la estructura del conjunto completo de módulos para reducir de forma importante el coste del análisis.
\\
\\
...
\\
\\
Además, cuando el análisis se lleva a conjuntos realmente grandes, entran en juego problemas más prácticos de ingeniería \cambio{debido al coste computacional}. En la ejecución descrita en la tesis, el experimento principal tuvo que dividirse en 13 lotes de alrededor de 12 millones de claves cada uno, y el proceso completo requirió del orden de 180 GB de memoria RAM, aproximadamente 1 TB de almacenamiento en disco y unas 88 horas de ejecución utilizando 30 núcleos \citep{vage2022finding}. Estas cifras dejan claro que, aunque el método clásico funciona, escalarlo no resulta trivial y exige recursos considerables.
\\
\\
...
A la vista de lo revisado en este capítulo, \cambio{queremos finalizar situando la aportación de este trabajo en el estado del arte actual.} \commA{Lo del punto exacto creo que da a polémica}
%puede situarse con bastante claridad el punto exacto en el que se encuentra este Trabajo de Fin de Máster dentro de la literatura previa.
\\
\\
