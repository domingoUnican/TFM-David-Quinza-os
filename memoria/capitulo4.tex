...
\\
\\
Además, se ha trabajado con un flujo de preparación de datos reales procedentes de \textit{Certificate Transparency}. Inicialmente se utilizó una muestra local ya extraída y normalizada, lo que permitió validar que el binario era capaz de leer módulos RSA reales, eliminar duplicados y ejecutar el algoritmo completo. Sin embargo, el objetivo aplicado del trabajo requiere ir más allá de esa muestra y preparar un proceso que pueda alimentarse directamente de colecciones mucho mayores de certificados.
\\
\\
Para ello se ha utilizado como fuente de datos el archivo público \textit{ct-archive}, que recopila volcados de distintos \textit{Certificate Transparency logs} en forma de archivos comprimidos. En lugar de descargar y conservar de manera permanente todos los certificados, el flujo diseñado trabaja de forma incremental: se descarga un archivo comprimido, se inspecciona su contenido, se extraen únicamente las entradas situadas en la carpeta \texttt{issuer/} o \texttt{issuers/}, y se obtiene de cada certificado X.509 su módulo RSA y su emisor. Esta decisión reduce el espacio necesario en disco y permite procesar los logs por partes, manteniendo únicamente los resultados intermedios necesarios para continuar el análisis.
\\
\\
...
\\
\\
Durante la extracción se mantiene también un registro del estado de cada archivo comprimido. Los archivos ya procesados, los que fallan y los que aparentemente no contienen entradas \texttt{issuer/} se registran por separado. Esta distinción es importante porque, en la práctica, un archivo descargado de forma incompleta puede parecer inválido o no contener las rutas esperadas aunque el problema real sea una descarga parcial. Por este motivo, los casos marcados como ausencia de \texttt{issuer/} se revisan de forma conservadora antes de descartarlos definitivamente. Si el archivo local no es un ZIP válido, se conserva como evidencia, se elimina su marca de procesado y se vuelve a descargar.
\\
\\
...
\\
\\
Este paso es importante porque conecta la parte algorítmica con el objetivo aplicado del TFM. La implementación no se plantea únicamente como una reproducción de un experimento sintético, sino como la base para ejecutar el ataque \textit{all-to-all GCD} sobre certificados reales. En el entorno local se utiliza una muestra ya normalizada para validar el procedimiento completo, mientras que la extracción incremental de módulos desde \textit{CT logs} prepara la entrada que se ejecutará posteriormente en Altamira sobre un conjunto mucho mayor.
\[
...
\\
\\
Por tanto, la implementación desarrollada en este TFM debe entenderse como una traducción práctica del algoritmo \textit{Binary Tree Batch GCD} a un entorno eficiente de ejecución, validada frente a las implementaciones Python de referencia y adaptada tanto a experimentos controlados con datos sintéticos como a módulos RSA reales extraídos de \textit{CT logs}. Sobre esta implementación se apoyan los resultados que presentaremos en el capítulo siguiente y la campaña de ejecución preparada para Altamira.
