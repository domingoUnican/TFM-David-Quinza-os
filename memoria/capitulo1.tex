...
\\
\\
Según \cite{aumasson2024serious}, un certificado es una clave pública acompañada de una firma de esta clave pública, además de otra información asociada. Como veremos, el uso de estos certificados en Internet permite un uso de la Web seguro y confiable. Por ejemplo para que un navegador web sepa que una clave pública es segura, es necesario que pueda verificar que esta pertenece a la entidad con la que se quiere comunicar (por ejemplo www.unican.es). En este caso es necesario que haya una firma electrónica que verifique que el dominio es correcto, siendo este el papel de entidad de certificación o «certificate authority» (CA). En el caso de la Universidad de Cantabria, la entidad certificadora puede consultarse desde el propio navegador web (ver Figura~\ref{fig:ejemplo_certificado}), ya que los navegadores incluyen de serie los certificados raíz de las autoridades reconocidas.

\begin{figure}[htbp]
\centering
\includegraphics[scale=0.51]{figuras/Ejemplo certificado.png}
\caption{Ejemplo de entidad de certificación que certifica la autenticidad del dominio unican.es}
...
El resto del documento se  ha organizado del siguiente modo. El
Capítulo~2 presenta los fundamentos matemáticos necesarios, así como la propuesta del algoritmo RSA. Asimismo se introducen las garantías de seguridad que pueden llevar a la vulnerabilidad causada por la reutilización de factores primos y  por último se presentan los detalles técnicos de los certificados en el proyecto \textit{Certificate Transparency}. El
Capítulo~3  se centra en  la revisión de los principales trabajos previos relacionados con la
detección de claves RSA vulnerables. El Capítulo~4 presenta herramientas utilizadas, el esquema de \textit{Binary Tree Batch GCD} así la implementación propia desarrollada y la metodología que se ha seguido para la preparación de los datos. El Capítulo~5 expone y analiza los resultados
