...
\item Desencriptación: Una función \textit{Dec}$(c,s_k)$, que dado $C$ devuelva de nuevo el mensaje $m$.
\end{itemize}
...
\\
\\
Esta situación puede deberse a varias causas. Entre ellas se encuentran el uso incorrecto de librerías de generación aleatoria, errores de implementación o la falta de una fuente externa suficiente de entropía en el momento de generar la clave \cambio{ dado que se utilizan algoritmos deterministas para la búsqueda que dependen de una semilla o \textit{seed} aleatoria para producir distintos resultados, en el caso de que estos valores se repitan conducirán a los mismos resultados en la búsqueda de candidatos a primos. } \commA{Esto mira a ver como lo redactas mejor, creo que está bien dar una idea de porque es necesaria la aleatoriedad, es muchas veces que están reutilizando la misma semilla aleatoria.} \cambio{Aunque existen recomendaciones que describen el proceso para validar las fuentes de la entropía  como el reporte técnico como NIST Special Publication 800-90B \citep{NST800-90B}}\commA{Aquí he puesto una citación por completar, sobre el problema en fuentes de entropía}, este problema resulta especialmente preocupante en dispositivos con recursos limitados, sistemas embebidos o equipos que generan claves en condiciones poco favorables, por ejemplo justo después del arranque \cambio{en el que los valores generados son predecibles}.\commA{Esto último me imagino que es porque por ejemplo si miras el estado de la memoria al arrancar es predecible, pero no después de cierto tiempo de funcionamiento, si se te ocurre algún ejemplo ilustrativo breve, por favor añádelo para que se entienda mejor la idea.} 
\\
\\
...
\\
\\
\cambio{Queremos remarcar la importancia de la entropía en este trabajo como origen de la vulnerabilidad que queremos estudiar. El número de certificados ha aumentado exponencialmente con la conexión de nuevos dispositivos a la red,  en muchos casos que generan su par de claves RSA al arrancar el sistema. Debido a la dificultad de detectar estos comportamientos indeseados por el gran número de equipos que se conectan automáticamente a la red a auditar, una medida razonable es analizar grandes volúmenes de claves públicas en busca de factores compartidos para garantizar la seguridad de los mecanismos de certificación.} \commA{Creo que conviene aclar porque se hacen las comprobaciones después y no equipo a equipo pero redáctalo a tu gusto}
 Si la generación de claves no se realiza siempre en condiciones ideales, entonces la auditoría empírica de la infraestructura criptográfica desplegada en Internet se convierte en una tarea especialmente relevante desde el punto de vista de la ciberseguridad.
...
Para poder estudiar este problema en sistemas reales es necesario contar con una fuente \cambio{ de claves públicas que sea contenga un volumen de datos suficiente y que sea accesible}. En este trabajo esa fuente serán los \textit{Certificate Transparency logs}, o \textit{CT logs}, que almacenan certificados digitales emitidos para dominios y servicios de Internet.
\\
\\
Antes de explicar su utilidad, conviene recordar brevemente qué papel desempeñan las claves y los certificados digitales en la Web. Cada servidor conserva una clave privada que \cambio{debe mantenerse secreta e idealmente} no debe abandonar el sistema. \cambio{La}  clave pública correspondiente \cambio{a esta clave secreta es necesaria} para que otras partes puedan verificar operaciones criptográficas asociadas \cambio{mediante mecanismos de firma digital}. Un certificado digital, normalmente en formato X.509, vincula esa clave pública con una identidad o dominio. Una autoridad certificadora comprueba dicha vinculación y firma el certificado con su propia clave privada; el cliente puede verificar la firma mediante la clave pública de la autoridad, directamente o a través de una cadena de confianza.
\\
\\
\cambio{El formato X.509 ha funcionado como estándar de facto para el manejo de certificados en las infraestructuras de clave pública, propuesto inicialmente en 1988 como parte de la serie X.500 por la Unión Internacional de Telecomunicaciones (ITU por sus siglas en inglés) para la interconexión de sistemas abiertos\cite{ITU1988X500}. Estos certificados contienen   incluye la siguiente información estructurada: la versión, el número de serie, Identificador del algoritmo utilizado por la autoridad para firmar (típicamente RSA o DSA), el emisor (CA), la validez temporal, el sujeto o sujeto titular expresado en notación DN (Distinguished Name) , la información de clave pública del sujeto que contiene el algoritmo de clave pública y la clave pública del sujeto, el algoritmo usado para firmar el certificado y la firma digital del certificado.} \commA{Esto por favor, revísalo, si quieres pon alguna imagen con la estructura o un ejemplo que tiene uno de estos certificados para ver que es fácil de extraer la información.}
...
\\
\\
Esto conecta de forma directa con la línea de trabajo seguida en este trabajo. Gracias a los \textit{CT logs} es posible pasar de un análisis puramente teórico sobre debilidades de RSA a una auditoría empírica sobre claves públicas realmente desplegadas en Internet. Como veremos en el capítulo siguiente, esta posibilidad ha dado lugar a trabajos previos muy relevantes y justifica la necesidad de seguir buscando métodos más eficientes para detectar factores compartidos a gran escala. \commA{He comentado lo siguiente porque pienso que está mejor en el estado del arte, pero me imagino que lo conecte, si quieres comentar algo más sería sobre los CT logs.}
