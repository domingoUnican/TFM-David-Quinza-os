...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \draw[flow] (r12) -- node[right, font=\small]
            {\(R_1=R_{12}\bmod N_1^2\)} (g12);
        \draw[flow] (r34) -- node[right, font=\small]
            {\(R_3=R_{34}\bmod N_3^2\)} (g34);
    ...
    productos y un árbol de residuos. Solo se muestran dos de las cuatro
    comprobaciones finales para mantener legible la figura.}
...
para construir el nodo padre. Este procedimiento se aplica de forma recursiva hasta alcanzar la parte superior del árbol \citep{pelofske2024efficient}.
...
Cualquier resultado no trivial obtenido en esta enumeración indica que el módulo \(N_i\) comparte algún factor con otro módulo del conjunto. De este modo, el algoritmo transforma toda la información agregada en \(B\) en factores concretos asociados a módulos individuales \citep{pelofske2024efficient}.
\\
\\
...
La implementación principal utilizada en este trabajo se ha desarrollado en C++17 empleando la biblioteca GMP para la aritmética multiprecisión \citep{gmp2023manual}. La elección de C++ responde a la necesidad de trabajar con enteros muy grandes de manera eficiente y de disponer de un mayor control sobre el uso de memoria y el rendimiento general de la ejecución. El código está preparado para aprovechar OpenMP \citep{openmp2021specification} cuando el entorno lo permite, especialmente en la fase de enumeración final, aunque conserva la posibilidad de ejecutarse de forma secuencial si el sistema no dispone de esta dependencia.
\\
\\
