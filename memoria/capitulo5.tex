...
unificada. El sistema utilizado fue macOS sobre el kernel Darwin. El Cuadro~\ref{tab:entorno_experimental} resume las principales versiones del
...
tiempo de pared \cambio{(\textit{wall time}), esto es} desde el lanzamiento hasta la finalización del proceso. El consumo de memoria se estimó mediante el máximo de memoria residente
...
\begin{figure}[htbp]
    ...
    \caption{Comparación del consumo pico de memoria de los tres algoritmos evaluados. en verde Binary Tree(C++/GPP), en azul Binary Tree (Python) y en rojo la implementación clásica en Python.}
...
\\
\\
Este comportamiento coincide con \cambio{ lo reportado en la literatura}, el algoritmo \textit{Binary Tree Batch GCD} resulta especialmente ventajoso cuando el número de factores compartidos es pequeño en comparación con el tamaño total del conjunto. A medida que aumenta el número de módulos débiles, la ventaja se reduce parcialmente, aunque el algoritmo sigue siendo claramente más eficiente que el enfoque clásico en todas las configuraciones evaluadas.
...
La distribución por tamaño de los módulos plausibles se muestra en el  Cuadro~\ref{tab:distribucion_bits_reales}. La mayoría de los módulos corresponden a claves RSA de 2048 bits, aunque también aparecen claves de 1024, 3072 y 4096 bits, además de algunos tamaños ligeramente distintos.
